...
- (void)setupWithTitle:(NSString *)titleText message:(NSString *)messageText accessoryView:(UIView *)accessoryView buttonTitle:(NSString *)buttonTitle {
    ...
    _messageLabel.textColor = [WPStatsStyleGuide allTAllShadeGrey];
    ...
    if (buttonTitle.length > 0) {
        ...
        [_button setTitleColor:[WPStatsStyleGuide allTAllShadeGrey] forState:UIControlStateNormal];
        [_button.titleLabel setFont:[WPStatsStyleGuide regularTextFont]];
        ...
        CGContextSetStrokeColorWithColor(context, [WPStatsStyleGuide allTAllShadeGrey].CGColor);
...
- (void)setTitleText:(NSString *)title {
    if (title.length > 0) {
        _titleLabel.attributedText = [[NSAttributedString alloc] initWithString:title attributes:[WPNUXUtility titleAttributesWithColor:[WPStatsStyleGuide whisperGrey]]];
    }
    [self setNeedsLayout];
}
